...
type PatternStateMachineTableProps = {
	pattern: string;
	tokens: string[];
	entriesByToken: Record<string, PatternLifecycleEntry[]>;
	nowMs: number;
};
...
export const PatternStateMachineTable = ({
	...
	type Row = { token: string; entry: PatternLifecycleEntry | undefined };
	const rows: Row[] = tokens.flatMap((token): Row[] => {
		const entries = entriesByToken[token] ?? [];
		if (entries.length === 0) {
			return [{ token, entry: undefined }];
		}
		return entries.map((entry) => ({ token, entry }));
	});
				...
				<table className="w-full min-w-[640px] border-separate border-spacing-y-3 text-left">
					...
					<tbody>
						{rows.map(({ token, entry }, index) => {
							...
							return (
								<tr
									key={`${pattern}-${token}-${entry?.interval ?? "none"}-${index}`}
