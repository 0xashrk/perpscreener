...
const dominantTone = (summary: PatternSummary) => {
	if (
		summary.bullishScore >= summary.bearishScore &&
		summary.bullishScore >= summary.neutralScore
	) {
		return {
			label: "Bullish",
			tone: "text-emerald-600",
			value: summary.bullishScore,
		};
	}
	if (summary.bearishScore >= summary.neutralScore) {
		return {
			label: "Bearish",
			tone: "text-rose-600",
			value: summary.bearishScore,
		};
	}
	return {
		label: "Neutral",
		tone: "text-slate-500",
		value: summary.neutralScore,
	};
};
	...
	return (
		...
		>
			...
			<div className="mt-2 flex items-center justify-between text-xs text-slate-500">
				<span className={`font-semibold ${dominant.tone}`}>
					{dominant.label}
				</span>
...
export const PatternSummaryPanel = ({
	summaries,
	...
	const [orderedSummaries, setOrderedSummaries] =
		useState<PatternSummary[]>(initialSorted);
	...
	const syncedSummaries = orderedSummaries
		...
		.map(
			(s) => summaries.find((ns) => getSummaryId(ns) === getSummaryId(s)) ?? s,
		);

	// Append any new summaries not in current order
	const newSummaries = summaries.filter(
		(s) => !orderedIds.has(getSummaryId(s)),
	);
	const finalSummaries = [...syncedSummaries, ...newSummaries];

	if (
		finalSummaries.length !== orderedSummaries.length ||
		finalSummaries.some(
			(s, i) => getSummaryId(s) !== getSummaryId(orderedSummaries[i]),
		)
	) {
	...
	const handleCapture = useCallback(async () => {
		...
		try {
			...
			if (navigator.share && navigator.canShare) {
				...
				const file = new File([blob], "pattern-summary.png", {
					type: "image/png",
				});
